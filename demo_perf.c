...
#define NUM_CONCURRENT_REQUESTS 4
#define NUM_ITERATIONS 10000

#define DO_SELECTS 0
#define USE_PREPARED 0

const char* value_string = "012345670123456701234567012345670123456701234567";
...
void insert_into_perf(CassSession* session, const char* query, const CassPrepared* prepared) {
  ...
  for (i = 0; i < NUM_CONCURRENT_REQUESTS; ++i) {
    ...
    key1 = rand() % 1000000;
    key2 = rand() % 5;
    cass_statement_bind_int64(statement, 0, (cass_int64_t) key1);
    cass_statement_bind_int64(statement, 1, (cass_int64_t) key2);
    cass_statement_bind_string(statement, 2, value_string);
...
void run_insert_queries(void* data) {
  ...
  const char* insert_query = "INSERT INTO vrts.table1 (key1, key2, value) VALUES (?, ?, ?);";
...
void select_from_perf(CassSession* session, const char* query, const CassPrepared* prepared) {
  ...
  for (i = 0; i < NUM_CONCURRENT_REQUESTS; ++i) {
    ...
    if (rc != CASS_OK) {
      ...
      assert(cass_result_column_count(result) == 3);
...
void run_select_queries(void* data) {
  ...
  const char* select_query = "SELECT * FROM vrts.table1 WHERE key1 = ?";
...
int main(int argc, char* argv[]) {
  ...
  char* hosts = "192.168.2.19,192.168.2.20,192.168.2.21";
  //char* hosts = "192.168.2.21";
  if (argc > 1) {
    hosts = argv[1];
  }
  ...
  execute_query(session, "CREATE KEYSPACE IF NOT EXISTS vrts WITH "
                         "replication = { 'class': 'SimpleStrategy', 'replication_factor': '3'}");

  execute_query(session, "CREATE TABLE IF NOT EXISTS vrts.table1 (key1 bigint, "
                         "key2 bigint, value text, PRIMARY KEY(key1, key2)) "
  ...
  cass_session_get_metrics(session, &metrics);
  printf("final stats (microseconds): min %llu max %llu mean %llu median %llu 75th %llu 95th %llu 98th %llu 99th %llu 99.9th %llu\n",
         (unsigned long long int)metrics.requests.min, (unsigned long long int)metrics.requests.max,
         (unsigned long long int)metrics.requests.mean,
